...
export default function ProjectHeader({ project, onUpdate, setToast }) {
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid var(--border)', padding: '28px 32px' }}>
        <div style={{ maxWidth: '900px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
            <button className="btn-primary" onClick={handleSave} disabled={saving}>{saving ? 'Saving...' : '✓ Save Details'}</button>
            <button className="btn-ghost" onClick={() => { setEditing(false); setForm({ ...project }); }}>Cancel</button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div style={{ background: 'linear-gradient(135deg, var(--wine) 0%, #7a2236 100%)', padding: '32px', color: '#fff', position: 'relative' }}>
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '16px', flexWrap: 'wrap' }}>
          <div style={{ flex: 1 }}>
            ...
            {project.client_specialty && (
              <p style={{ opacity: 0.8, fontSize: '0.9rem', marginBottom: '8px' }}>{project.client_specialty}</p>
            )}
            ...
            <div style={{ background: 'rgba(255,255,255,0.1)', borderRadius: '10px', padding: '16px 20px', display: 'inline-block', backdropFilter: 'blur(10px)', border: '1px solid rgba(255,255,255,0.15)' }}>
              <div style={{ fontSize: '0.72rem', opacity: 0.65, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '4px' }}>Project</div>
              <div style={{ fontFamily: 'Lora, serif', fontSize: '1.05rem', fontWeight: 600, marginBottom: '10px' }}>{project.project_name || 'Project Name'}</div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '12px' }}>
            <span style={{ ...statusStyle, padding: '6px 16px', borderRadius: '20px', fontSize: '0.78rem', fontWeight: 600 }}>
              {project.project_status || 'In Progress'}
            </span>
            {/* Only show Edit button to admin */}
            {isAdmin && (
              <button className="btn-ghost" style={{ borderColor: 'rgba(255,255,255,0.3)', color: '#fff', fontSize: '0.8rem' }} onClick={() => setEditing(true)}>
                ✎ Edit Details
              </button>
            )}
          </div>
