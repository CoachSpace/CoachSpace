...
export default function WelcomeTab({ project, setToast }) {
  ...
  const embedUrl = getEmbedUrl(videoLink || project.video_link);
        ...
        {editingWelcome ? (
          <div>
            <div style={{ marginBottom: '16px' }}>
              ...
              <textarea
                ...
                rows={5}
                placeholder="Write a warm welcome message for your client..."
            ...
            <div style={{ marginBottom: '16px' }}>
              ...
              <input
                ...
                placeholder="https://www.loom.com/share/... or YouTube URL"
              />
            </div>
            <div style={{ display: 'flex', gap: '10px' }}>
              <button className="btn-primary" onClick={saveWelcome} disabled={saving}>
                {saving ? 'Saving...' : '✓ Save'}
              </button>
              <button className="btn-ghost" onClick={() => { setEditingWelcome(false); setWelcomeMsg(project.welcome_message || ''); setVideoLink(project.video_link || ''); }}>
                Cancel
              </button>
            </div>
          </div>
          ...
          <div>
            <p style={{ color: 'var(--text)', lineHeight: 1.7, fontSize: '0.92rem', whiteSpace: 'pre-wrap' }}>
              {project.welcome_message || <span style={{ color: 'var(--rose)', fontStyle: 'italic' }}>No welcome message yet. Click Edit to add one.</span>}
            </p>
          </div>
        )}
      </div>
      ...
      {(project.video_link || videoLink) && (
        <div className="card">
          ...
          {embedUrl ? (
            ...
            <a href={project.video_link || videoLink} target="_blank" rel="noopener noreferrer"
              style={{
                display: 'flex', alignItems: 'center', gap: '12px',
                padding: '16px', background: 'var(--cream)',
                borderRadius: '10px', border: '1.5px solid var(--border)',
                color: 'var(--wine)', textDecoration: 'none', fontSize: '0.9rem',
              }}>
      ...
      <div className="card">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          <div>
            ...
            <p style={{ color: 'var(--rose)', fontSize: '0.8rem', marginTop: '4px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {checklist.map(item => (
              <div key={item.id} style={{
                ...
                gap: '12px',
                padding: '12px 14px',
                background: item.is_completed ? '#f0faf4' : 'var(--cream)',
                borderRadius: '8px',
                border: `1.5px solid ${item.is_completed ? '#b7dfca' : 'var(--border)'}`,
                transition: 'all 0.2s',
              }}>
                <button
                  ...
                  style={{
                    ...
                    borderRadius: '50%',
                    border: `2px solid ${item.is_completed ? '#2d8a54' : 'var(--border)'}`,
                    background: item.is_completed ? '#2d8a54' : '#fff',
                ...
                <span style={{
                  ...
                  color: item.is_completed ? '#6b8f7a' : 'var(--text)',
                }}>
                  {item.label}
                </span>
                <button
                  onClick={() => deleteItem(item.id)}
                  style={{ background: 'none', border: 'none', color: 'var(--border)', cursor: 'pointer', fontSize: '16px', lineHeight: 1 }}
                >
                  ×
                </button>
              </div>
